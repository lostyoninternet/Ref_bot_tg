...
При включённом шифровании (см. ниже) в ссылке передаются **короткие токены**, а не открытые email и телефон:

| Параметр      | Значение        | Описание                    |
|---------------|-----------------|-----------------------------|
| `utm_source`  | `referal`       | Источник — реферальная программа |
| `utm_medium`  | ник в Telegram  | Username участника (без @)  |
| `utm_campaign`| токен email     | Короткий токен (например `a3Fk9xK2`) |
| `utm_content` | токен телефона  | Короткий токен (например `mN7pQ1zR`) |

Пример с токенами:  
`https://polytech.alabuga.ru/?utm_source=referal&utm_medium=username&utm_campaign=a3Fk9xK2&utm_content=mN7pQ1zR`

Реферальная ссылка доступна только после указания **email** и **номера телефона** (при регистрации можно «Поделиться контактом»).

## Шифрование и ключ для Битрикса

В БД хранятся **только зашифрованные** email и телефон. В UTM — короткие токены. Для расшифровки выгрузки из Битрикса в Excel используется файл-ключ:

- В админке при экспорте выдаётся два файла: выгрузка пользователей (с расшифрованными контактами) и **utm_key_*.csv** — соответствие «токен → расшифрованное значение».
- В Excel по этому ключу можно подставить реальные email/телефон вместо токенов (VLOOKUP и т.п.).

Подробно: [docs/ENCRYPTION_AND_UTM.md](docs/ENCRYPTION_AND_UTM.md). В `.env` нужно задать `ENCRYPTION_KEY` (64 hex-символа или строка ≥32 символов).
...
3. Создайте `.env` из `.env.example` и заполните:
   ...
   - `ENCRYPTION_KEY` — ключ шифрования (64 hex-символа или строка ≥32 символов); без него в UTM и БД будут открытые данные
