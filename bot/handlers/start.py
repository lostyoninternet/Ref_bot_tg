...
async def cmd_start(message: Message, bot: Bot, state: FSMContext):
    ...
    if created or not has_email:
        ...
        await message.answer(
            f"🎉 Отлично, {first_name or 'друг'}! Очный этап пройден!\n\n"
            "Чтобы учавствовать в реферальной программе и получать награды, нам нужно связать твою анкету с ботом.\n\n"
            "<b>📩 Введи свой EMAIL ниже:</b>\n\n"
            "(Используй тот же адрес, который указывал при регистрации на Очный этап)\n\n",
            parse_mode="HTML"
        )
        return
...
async def _save_phone_and_finish(message: Message, state: FSMContext, phone: str, bot: Bot):
    ...
    await message.answer(
        ...
        "Самое приятное — уже за 5 человек ты переходишь на 1 грейд и получаешь классный рюкзак 🎒",
        parse_mode="HTML",
        reply_markup=reply_kb,
    )
